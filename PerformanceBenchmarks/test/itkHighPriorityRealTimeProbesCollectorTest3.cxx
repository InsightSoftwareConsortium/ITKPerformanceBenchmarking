...
int main(int, char* [] )
{
  ...
  collector.EnableUpdatingProbeNameWithNumOfThreads(true);
  ...
  unsigned int max_numOfThread(3);

  for(int numthread = 1; numthread <= max_numOfThread; numthread++)
    {
    collector.SetNumberOfThreads(numthread);
  ...
  //collector.Report();
  collector.ExpandedReportAll();
  collector.AnalysisReportAll();

  // Test writing to a ostream
//  std::ofstream  logfile;
//  logfile.open("HighPriorityRealTimeProbesCollectorTest3.txt");
//  collector.ExpandedReportAll( logfile );
//  logfile.close();

//  // Print to the standar error
//  collector.ExpandedReportAll( std::cerr );
