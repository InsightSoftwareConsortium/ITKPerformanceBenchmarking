...
int main( int argc, char * argv[] )
{
  ...
  unsigned int numberOfIteration = 30;
// Software Guide : BeginLatex
//
...
// Software Guide : BeginCodeSnippet
  ...
  // Inverse transform = forward transform / num voxels
  ...
  // Input Image
  IOImageType::Pointer inputImage = inputreader->GetOutput();
  inputImage->DisconnectPipeline();

  // Set up input image of inputresample
  inputresampler->SetInput(inputImage);
  ...
  {
    // InputResample
    collector.Start("InputResample");
    inputresampler->UpdateLargestPossibleRegion();
    collector.Stop("InputResample");

    // Forward FFT filter
    fftinput->SetInput( inputresampler->GetOutput() );
    collector.Start("FFTFilter");
    fftinput->UpdateLargestPossibleRegion();
    collector.Stop("FFTFilter");

    // Do the inverse transform
    fftoutput->SetInput(fftinput->GetOutput()); // try to recover the input image
    collector.Start("invFFTFilter");
    fftoutput->UpdateLargestPossibleRegion();
    collector.Stop("invFFTFilter");

    // OutputResample
    outputResampler->SetDefaultPixelValue( 0 );
    outputResampler->SetSize( inputsize );
    outputResampler->SetInput( fftoutput->GetOutput() );
    collector.Start("OutputResample");
    outputResampler->UpdateLargestPossibleRegion();
    collector.Stop("OutputResample");
