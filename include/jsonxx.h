...
std::string
xml(const std::string & input, unsigned format = JSONx);
std::string
xml(std::istream & input, unsigned format = JSONx);

// Detail
void
assertion(const char * file, int line, const char * expression, bool result);
...
public:
  ...
  void
  import(const Object & other);
  void
  import(const std::string & key, const Value & value);
  Object &
  operator<<(const Value & value);
  Object &
  operator<<(const Object & value);
  Object &
  operator=(const Object & value);
...
protected:
  static bool
              parse(std::istream & input, Object & object);
...
protected:
  static bool
            parse(std::istream & input, Array & array);
  container values_;
};
...
template <typename T>
const T &
Array::get(unsigned int i) const
{
  JSONXX_ASSERT(i < size());
